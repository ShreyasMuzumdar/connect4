...
def prompt_for_starting_player():
    """Block until the player chooses who starts and return the matching turn constant."""
    title_font = pygame.font.SysFont("monospace", 60)
    option_font = pygame.font.SysFont("monospace", 32)
    ...
    while True:
        ...
        screen.fill(BLACK)
        title_surface = title_font.render("Who goes first?", True, YELLOW)
        player_surface = option_font.render("Press 1: You start", True, RED)
        ai_surface = option_font.render("Press 2: AI starts", True, YELLOW)

        screen.blit(title_surface, (width // 2 - title_surface.get_width() // 2, height // 2 - 80))
        screen.blit(player_surface, (width // 2 - player_surface.get_width() // 2, height // 2 - 10))
        screen.blit(ai_surface, (width // 2 - ai_surface.get_width() // 2, height // 2 + 40))
...
# initial turn is chosen by the player via a start dialog
turn = None
...
# draw GUI
turn = prompt_for_starting_player()
draw_board(board)
pygame.display.update()
